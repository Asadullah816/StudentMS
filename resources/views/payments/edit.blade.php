...
<div class="content">
    ...
    <div class="container mt-5">
        <form action="{{ route('paymentEdit', $payment->id) }}" method="POST">
            @if ($errors->any())
            ...
            <div class="form">
                <div class="bg-secondary rounded h-100 p-4">
                    ...
                    <select name="enrollment_id" class="form-select form-select-lg mb-3"
                        ...
                        <option value="{{ $payment->enrollment->enroll_no }}" selected>
                            {{ $payment->enrollment->student->name }}</option>
                        @foreach ($enrollment as $data)
                            <option value="{{ $data->id }}">{{ $data->student->name }}</option>
                        @endforeach
                    </select>

                    <div class="form-floating mb-3">
                        <input type="number" class="form-control" value="{{ $payment->ammount }}" name="ammount"
